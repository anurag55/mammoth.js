...
function BodyReader(options) {
    ...
    function readParagraphStyle(element) {
        var res = readStyle(element, "w:pStyle", "Paragraph", styles.findParagraphStyleById);
        return res;
    }
    ...
    function readTableRow(element) {
        ...
        var height = properties.firstOrEmpty("w:trHeight").attributes["w:val"]
        if(height)
          height = convertDxaToPt(height)
    ...
    function readTableCell(element) {
        return readXmlElements(element.children).map(function(children) {
            ...
            var vAlign = properties.firstOrEmpty("w:vAlign").attributes["w:val"];
            /*
            ...
            */
            var widthElement = properties.firstOrEmpty("w:tcW");
            var width;
            if(widthElement.attributes["w:w"]) {
              if(widthElement.attributes["w:type"] == "pct") {
                width = widthElement.attributes["w:w"] 
              } else if(widthElement.attributes["w:type"] == "dxa") {
                // convert to px 
                // 20 dxa  = 1pt
                width = convertDxaToPt(widthElement.attributes["w:w"])
              }
            }

            //   <w:shd w:val="clear" w:color="auto" w:fill="EEECE1" w:themeFill="background2" />
            var bgColor = properties.firstOrEmpty("w:shd").attributes["w:fill"];
            if(bgColor) 
              bgColor = "#" + bgColor
            
            var cell = documents.TableCell(children, { width: width, colSpan: colSpan,  vAlign: vAlign, bgColor: bgColor});
