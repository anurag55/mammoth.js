...
function DocumentConversion(options, comments) {
    ...
    function htmlPathForParagraph(element, messages) {
        ...
        if (style) {
            ...
            return htmlPaths.topLevelElement('p', convertStyleForaAttributes(element));
        }
    }
    ...
    function convertRun(run, messages, options) {
        ...
        var paths = [];
        var tagNumber = 0;
        var attributes = convertStyleForaAttributes(run);
        if (run.isSmallCaps) {
        ...
        if (run.isItalic) {
            paths.push(findHtmlPathForRunProperty("italic"));
        }
        if (run.isBold) {
            paths.push(findHtmlPathForRunProperty("bold"));
        }
        if (run.bgColor) {
            paths.push(htmlPaths.element("mark", attributes, {fresh: false}));
            tagNumber++;
        }
    ...
    function convertStyleForaAttributes(element, options) {
        var styleMap = [];
        if (!options) {
            options = {};
        }
        if (element.alignment) {
            const alignment = element.alignment == 'both' ? 'justify' : element.alignment;
            styleMap.push({key: 'text-align', value: alignment});
        }
        if (element.color) {
        ...
        if (element.isBold) {
            styleMap.push({key: 'font-weight', value: 'bold'});
        }
        if (element.isItalic) {
            styleMap.push({key: 'font-style', value: 'italic'});
        }
        ...
        if (element.lineHeight) {
            styleMap.push({key: 'line-height', value: element.lineHeight});
        }

        if (element.indent && element.indent.firstLine) {
            styleMap.push({key: 'text-indent', value: parseInt(element.indent.firstLine, 10) / 210 + 'em'});
        }
    ...
    function convertTableRow(element, messages, options) {
        ...
        if (element.height) {
            attributes.style = attributes.style + "height:" + element.height + ";";
        }
        return [
    ...
    function convertTableCell(element, messages, options) {
        ...
        if (element.verticalAlignment) {
            attributes.style = attributes.style + "vertical-align:"
            attributes.style += element.verticalAlignment === "center"
                ? "middle"
        ...
        if (element.width) {
            attributes.style = attributes.style + "width:" + element.width + ";";
        }

        if (element.bgColor) {
            attributes.style = attributes.style + "background-color:" + element.bgColor + ";";
        }
